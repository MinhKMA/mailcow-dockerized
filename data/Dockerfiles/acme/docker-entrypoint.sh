...
while [[ -z ${DOMAIN_TABLE} ]]; do
  ...
  DOMAIN_TABLE=$(mysql --host=${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SHOW TABLES LIKE 'domain'" -Bs)
...
while true; do
  ...
  done < <(mysql --host=${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SELECT domain FROM domain WHERE backupmx=0" -Bs)
