...
sed -i "s/__DBHOST__/${DBHOST}/g" /usr/local/bin/imapsync_cron.pl
...
sed -i "s/__DBHOST__/${DBHOST}/g" /usr/local/bin/quarantine_notify.py
...
sed -i "s/__DBHOST__/${DBHOST}/g" /usr/local/bin/clean_q_aged.sh
...
IMAPSYNC_TABLE=$(mysql --host=${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SHOW TABLES LIKE 'imapsync'" -Bs)
[[ ! -z ${IMAPSYNC_TABLE} ]] && mysql --host=${DBHOST} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "UPDATE imapsync SET is_running='0'"
